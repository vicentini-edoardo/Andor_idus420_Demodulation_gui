...
class AndorShamrockBackend(SpectroBackend):
    ...
    def disconnect(self) -> None:
        if self._spc is not None:
            with contextlib.suppress(Exception):
                self._spc.Close()  # type: ignore[attr-defined]
        self._spc = None
        self._connected = False
        self._n_gratings = 0
    ...
    def list_gratings(self) -> list[GratingInfo]:
        ...
        for i in range(1, self._n_gratings + 1):
            ret, lines, blaze, home, offset = self._spc.GetGratingInfo(i)  # type: ignore[attr-defined]
    ...
    def get_grating(self) -> int:
        if not self._spc:
            raise SpectroError("Not connected.")
        ret, grating = self._spc.GetGrating()  # type: ignore[attr-defined]
    ...
    def set_grating(self, index: int) -> None:
        if not self._spc:
            raise SpectroError("Not connected.")
        ret = self._spc.SetGrating(index)  # type: ignore[attr-defined]
        _check(ret, f"SetGrating({index})")

    def get_wavelength(self) -> float:
        if not self._spc:
            raise SpectroError("Not connected.")
        ret, wl = self._spc.GetWavelength()  # type: ignore[attr-defined]
    ...
    def set_wavelength(self, nm: float) -> None:
        if not self._spc:
            raise SpectroError("Not connected.")
        ret = self._spc.SetWavelength(float(nm))  # type: ignore[attr-defined]
    ...
    def get_wavelength_limits(self) -> tuple[float, float]:
        ...
        ret, minwl, maxwl = self._spc.GetWavelengthLimits(grating)  # type: ignore[attr-defined]
    ...
    def get_calibration(self, n_pixels: int, pixel_width_um: float = 26.0) -> np.ndarray:
        if not self._spc:
            raise SpectroError("Not connected.")
        ret = self._spc.SetNumberPixels(int(n_pixels))  # type: ignore[attr-defined]
        _check(ret, "SetNumberPixels")
        ret = self._spc.SetPixelWidth(float(pixel_width_um))  # type: ignore[attr-defined]
        _check(ret, "SetPixelWidth")
        ret, cal = self._spc.GetCalibration(int(n_pixels))  # type: ignore[attr-defined]
